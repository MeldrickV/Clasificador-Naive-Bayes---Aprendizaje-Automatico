...
from gui.styles import COLORS, FONTS, PAD
from gui.widgets.components import (
    SectionHeader, InfoBox, MetricCard, Badge
)
...
class TrainTab(tk.Frame):
    ...
    def _build(self):
        ...
        inner = tk.Frame(action_bar, bg=COLORS["bg_card"])
        inner.pack(fill="x", padx=16, pady=12)

        ttk.Button(inner, text="  ▶  Entrenar Modelo",
        ...
        self._progress = ttk.Progressbar(inner, length=200,
                                         mode="indeterminate")
        self._progress.pack(side="left")

        # ── Tarjetas de métricas ───────────────────
        metrics_frame = tk.Frame(self, bg=COLORS["bg_main"])
        ...
        log_card = tk.Frame(self, bg=COLORS["bg_card"],
                            highlightbackground=COLORS["border"],
                            highlightthickness=1)
        ...
        SectionHeader(log_card, "Log de Entrenamiento",
                      ...
                      icon="📋",
                      bg=COLORS["bg_card"]).pack(fill="x", padx=14, pady=(12, 6))

        # Texto del log
        log_outer = tk.Frame(log_card, bg=COLORS["bg_code"])
        log_outer.pack(fill="both", expand=True, padx=14, pady=(0, 14))

        self.log_text = tk.Text(log_outer, bg=COLORS["bg_code"],
                                fg="#a8d8a8",
                                font=FONTS["mono"],
    ...
    def _set_status(self, text: str, style: str):
        self._status_badge.config(text=text,
                                  bg=Badge.STYLES[style][0],
                                  fg=Badge.STYLES[style][1])
